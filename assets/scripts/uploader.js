...
class ImageUpload extends HTMLElement {
  constructor() {
    ...
    const handleDragLeave = (event, shadow) => {
      event.preventDefault();
      shadow.querySelector("#dropZone")
        ?.setAttribute("style", dropzoneStyle);
    };
    /**
     * Handle file drop
     *
    ...
    dropZone.addEventListener('dragleave', (ev) => { handleDragLeave(ev, this._shadowRoot) })
    dropZone.addEventListener('dragover', (ev) => { handleDragOver(ev, this._shadowRoot) })
    dropZone.addEventListener('drop', (ev) => { handleDropFileChange(ev, this._shadowRoot) })
    ...
    inputElement.addEventListener('change', (ev) => { handleFileChange(ev, this._shadowRoot) });
